#!/usr/bin/env python3
"""Real-world integration demo: vDC host + vDC + vdSD announcement.

This script demonstrates the full lifecycle of a vDC host with an
announced vDC **and a vdSD device** against a real digitalSTROM system
(dSS / vdSM):
  ...
  **Phase 1 — Fresh start**
  ...
  6. Verify that auto-save has persisted state automatically.
  ...
  **Phase 2 — Restart from persistence**
  ...
  2. Verify that vDC, Device, and Vdsd are all restored.
  ...
  **Phase 3 — Vanish, shutdown & cleanup**
  ...
  1. Vanish the device/vdSD from the vdSM (per §6.3).
     Note: vDCs cannot vanish during a session (§5).
  2. Shut down and delete all persistence artefacts.

Run from the project root::

    python examples/realworld_test_vdsd.py
"""
...
async def main() -> None:
    ...
    # PHASE 1 — Fresh start
    # ==================================================================
    banner("PHASE 1: Fresh VdcHost + vDC + vdSD start")
    ...
    # Create a Device and a single Vdsd (light).
    # Derive a unique dSUID for the device from a stable name so it
    # remains the same across restarts.  Must differ from host and vDC.
    device_dsuid = DsUid.from_name_in_space(
        "demo-light-device-1", DsUidNamespace.VDC
    )
    ...
    # ------------------------------------------------------------------
    # Verify auto-save: the debounced timer (1 s) should have fired by
    # now — no explicit save() call is made.  This proves that the
    # auto-save mechanism persists the full state automatically.
    # ------------------------------------------------------------------
    ...
    await wait_for_user(
        ">>> vDC + vdSD announced, auto-save verified. "
        "Press Enter to shut down and proceed to Phase 2..."
    )

    # ------------------------------------------------------------------
    banner("PHASE 1: Shutting down")
    # stop() calls flush() internally — this captures any property
    # changes made between the auto-save above and now.  We do NOT
    # call save() explicitly; the whole point is to rely on auto-save.
    await host.stop()
    ...
    assert restored_vdsd.name == original_vdsd_name, (
        f"vdSD name mismatch! "
        f"{restored_vdsd.name!r} != {original_vdsd_name!r}"
    )
    ...
    # Keep connection alive — wait for user to terminate.
    await wait_for_user(
        ">>> Restored vDC + vdSD re-announced. Press Enter to "
        "vanish device and perform final shutdown..."
    )
    ...
    # Per §5 of the vDC API: "unlike individual devices, logical vDCs
    # cannot vanish during an established vDC session."  So we only
    # vanish the device/vdSD(s); the vDC itself stays until the session
    # ends when we call stop().

    # Properly de-announce/vanish the device before shutting down (§6.3).
    # Sends VDC_SEND_VANISH notification for each announced vdSD.
    session2 = host2.session
    ...
    logger.info("")
    logger.info("Summary:")
    logger.info("  Phase 1: Created host + vDC + device/vdSD, announced all,")
    logger.info("           auto-save persisted state (no explicit save)")
    logger.info("  Phase 2: Restored from auto-saved YAML, re-announced all")
